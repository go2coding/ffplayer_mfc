...
// ffplay_MFCDlg.cpp: 实现文件
//

#include "pch.h"
#include "framework.h"
#include "ffplay_MFC.h"
#include "ffplay_MFCDlg.h"
    ...
    do {
        ...
        if (got_frame) {
            ...
            while ((ret = av_buffersink_get_frame_flags(is->out_audio_filter, frame, 0)) >= 0) {
                ...
                AVRational avx;
                avx.num = frame->nb_samples, avx.den = frame->sample_rate;
                af->duration = av_q2d(avx);
...
#endif
            AVRational avxd;
            avxd.num = frame_rate.den, avxd.den = frame_rate.num;
            duration = (frame_rate.num && frame_rate.den ? av_q2d(avxd) : 0);
...
static int read_thread(void* arg)
{
    ...
    dlg->SetWindowPos(NULL, 0, 0, pCodecCtx->width, pCodecCtx->height + 120, SWP_NOMOVE);
    dlg->GetDlgItem(IDC_PICURE_CONTROL)->MoveWindow(0, 0, pCodecCtx->width, pCodecCtx->height);
    dlg->GetDlgItem(IDC_PLAY_PROGRESS)->MoveWindow(60, pCodecCtx->height + 10, pCodecCtx->width-160, dlg->m_rect_open_btn.bottom - dlg->m_rect_open_btn.top);

    dlg->GetDlgItem(IDC_EDIT1)->MoveWindow(10, pCodecCtx->height + 15, dlg->m_rect_open_btn.right - dlg->m_rect_open_btn.left-20, dlg->m_rect_open_btn.bottom - dlg->m_rect_open_btn.top);
    dlg->GetDlgItem(IDC_DURATION)->MoveWindow(pCodecCtx->width - 100, pCodecCtx->height + 15, dlg->m_rect_open_btn.right - dlg->m_rect_open_btn.left - 20, dlg->m_rect_open_btn.bottom - dlg->m_rect_open_btn.top);

    dlg->GetDlgItem(IDC_BTN_START)->MoveWindow(10, pCodecCtx->height + 50, dlg->m_rect_open_btn.right - dlg->m_rect_open_btn.left, dlg->m_rect_open_btn.bottom - dlg->m_rect_open_btn.top);
...
int ffmfc_play(LPVOID lpParam)
{
    ...
    if (!display_disable) {
        ...
        HWND hWnd = dlg->GetDlgItem(IDC_PICURE_CONTROL)->GetSafeHwnd();
...
CffplayMFCDlg::CffplayMFCDlg(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_FFPLAY_MFC_DIALOG, pParent)
{
...
void CffplayMFCDlg::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Control(pDX, IDC_EDIT1, m_currentclock);
...
BEGIN_MESSAGE_MAP(CffplayMFCDlg, CDialogEx)
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
    ON_BN_CLICKED(IDC_BTN_START, &CffplayMFCDlg::OnBnClickedBtnStart)
END_MESSAGE_MAP()
...
BOOL CffplayMFCDlg::OnInitDialog()
{
    ...
    GetDlgItem(IDC_BTN_START)->GetWindowRect(&m_rect_open_btn);//获取控件相对于屏幕的位置
...
void CffplayMFCDlg::OnBnClickedBtnStart()
{
    // TODO: 在此添加控件通知处理程序代码
...
BOOL CffplayMFCDlg::PreTranslateMessage(MSG* pMsg)
{
    ...
    if (pMsg->message == WM_KEYDOWN)
    {
        ...
        if (pMsg->wParam == VK_LEFT)
        {
            ...
            if (seek_by_bytes) {
                pos = -1;
                if (pos < 0 && cur_stream_all->video_stream >= 0)
                    pos = frame_queue_last_pos(&cur_stream_all->pictq);
                if (pos < 0 && cur_stream_all->audio_stream >= 0)
                    pos = frame_queue_last_pos(&cur_stream_all->sampq);
                if (pos < 0)
                    pos = avio_tell(cur_stream_all->ic->pb);
                if (cur_stream_all->ic->bit_rate)
                    incr *= cur_stream_all->ic->bit_rate / 8.0;
                else
                    incr *= 180000.0;
                pos += incr;
                stream_seek(cur_stream_all, pos, incr, 1);
            }
            else {
                pos = get_master_clock(cur_stream_all);
                if (isnan(pos))
                    pos = (double)cur_stream_all->seek_pos / AV_TIME_BASE;
                pos += incr;
                if (cur_stream_all->ic->start_time != AV_NOPTS_VALUE && pos < cur_stream_all->ic->start_time / (double)AV_TIME_BASE)
                    pos = cur_stream_all->ic->start_time / (double)AV_TIME_BASE;
                stream_seek(cur_stream_all, (int64_t)(pos * AV_TIME_BASE), (int64_t)(incr * AV_TIME_BASE), 0);
            }
